...
describe('API', () => {
	...
	describe('POST request for /api/reports', () => {
		...
		it('on caught error, call next(error), which sends back a 500 error', async () => {
			await expect(axios.post(`${API_URL}/api/reports`, { nothing: undefined })).rejects.toThrow(
	...
	describe('POST request for /api/reports/:reportId/comments', () => {
		beforeAll(async () => {});
		it('on caught error, call next(error), which sends back a 500 error', async () => {
			await expect(axios.post(`${API_URL}/api/reports/${2300}/comments`, { nothing: undefined })).rejects.toThrow(
				'Request failed with status code 500'
			);
		});
	...
	describe('DELETE request for /api/reports/:reportId', () => {
		...
		beforeAll(async () => {});
		it('on caught error, call next(error), which sends back a 500 error', async () => {
			await expect(axios.delete(`${API_URL}/api/reports/${2300}`)).rejects.toThrow(
				'Request failed with status code 500'
			);
		});
