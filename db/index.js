...
async function getOpenReports() {
  try {
    ...
    //note: for destructuring, if you want mutliple results back from a query
    //dont use [] around reports variable
    ...
    //load all comments
    //note: for destructuring, if you want mutliple results
    //back from a query then done use array around comments variable
    const {rows:comments} = await client.query(`
    select comments.id, comments."reportId", comments.content 
    from comments inner join reports 
    on comments."reportId" = reports.id;`)
    ...
    reports.forEach(report => {
      ...
      report.comments = comments.filter(comment => {
        return report.id === comment.reportId;
      })
...
async function closeReport(reportId, password) {
    ...
    try {
      ...
      if(password !== thatReport.password){
 ...
 */
   ...
   const updateReport =  await client.query(`
      ...
      where id = $1 and "password" = $2 RETURNING *`,[reportId,password])
...
const { content } = commentFields;
  ...
  try {
    ...
    if(Date.parse(thatReport.expirationDate) < new Date()) {
 ...
 */
    ...
    const { rows:[comment] } = await client.query(`
      INSERT INTO comments("reportId",content) VALUES ($1,$2)
      RETURNING *;
    `,[reportId,content])
